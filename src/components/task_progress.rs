...
//! ];
//!
//! let view = TaskProgressView::builder()
//!     .steps(steps)
...
// Builder is generated by bon via #[builder] on TaskProgressView::new()
...
impl TaskProgressView {
    /// Create a builder for a new TaskProgressView.
    ///
    /// Use the builder pattern for configuration:
    /// ```no_run
    /// use teapot::components::{TaskProgressView, TaskStep};
    ///
    /// let steps = vec![TaskStep::with_executor("Task", || Ok(None))];
    /// TaskProgressView::builder()
    ///     .steps(steps)
    ///     .title("My App")
    ///     .auto_start(true)
    ///     .build();
    ...
    pub fn new(
        ...
        #[builder(default)] subtitle: String,
        #[builder(default)] auto_start: bool,
        #[builder(default)] external_control: bool,
        #[builder(default)] hints_confirming: Vec<(String, String)>,
        #[builder(default)] hints_running: Vec<(String, String)>,
        #[builder(default)] hints_completed: Vec<(String, String)>,
        confirmation: Option<ConfirmationConfig>,
        context: Option<Box<dyn Any + Send + Sync>>,
    ) -> Self {
        let config = TaskProgressConfig {
            auto_start,
            external_control,
            hints: HintConfig {
                confirming: hints_confirming,
                running: hints_running,
                completed: hints_completed,
            },
        };

        let initial_phase = if confirmation.is_some() { Phase::Confirming } else { Phase::Ready };
...
mod tests {
    ...
    fn test_initial_phase_with_confirmation() {
        ...
        let view = TaskProgressView::builder()
            .steps(steps)
            .context(Box::new(()) as Box<dyn Any + Send + Sync>)
    ...
    fn test_initial_phase_without_confirmation() {
        let steps = vec![TaskStep::new("Step 1")];
        let view = TaskProgressView::builder().steps(steps).auto_start(true).build();
    ...
    fn test_external_control() {
        let steps = vec![TaskStep::new("Step 1")];
        let mut view = TaskProgressView::builder().steps(steps).external_control(true).build();
